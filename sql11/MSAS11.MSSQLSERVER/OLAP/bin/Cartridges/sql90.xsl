...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" version="1.0" xmlns:mssqlcrt="urn:sql-microsoft-com:sqlcrt" xmlns:mssqldbg="urn:sql-microsoft-com:sqldbg">
  ...
  <!-- SQL Server 90 pluggable cartridge                              -->
  ...
  RTRIM(LTRIM(<xsl:apply-templates select="./*[1]"/>))
	...
	<xsl:template match="OpExact">
		<xsl:variable name="op1">
			<xsl:choose>
				<xsl:when test="@Op1Nullable = 'false'"> (<xsl:apply-templates select="./*[1]"/>) </xsl:when>
				<xsl:otherwise> ISNULL(<xsl:apply-templates select="./*[1]"/>, '') </xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="op2">
			<xsl:choose>
				<xsl:when test="@Op2Nullable = 'false'"> (<xsl:apply-templates select="./*[2]"/>) </xsl:when>
				<xsl:otherwise> ISNULL(<xsl:apply-templates select="./*[2]"/>, '') </xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template match="OpConcatenate">
		<xsl:choose>
			<xsl:when test="@Op1Nullable = 'false' and @Op2Nullable = 'false'">
				CAST(<xsl:apply-templates select="./*[1]"/> AS nvarchar(max)) + (<xsl:apply-templates select="./*[2]"/>)
			</xsl:when>
			<xsl:when test="@Op1Nullable = 'false'">
				(SELECT ISNULL(CAST(AuxTable.Op1 AS nvarchar(max))+ AuxTable.Op2, AuxTable.Op1) FROM (SELECT (<xsl:apply-templates select="./*[1]"/>) AS Op1, (<xsl:apply-templates select="./*[2]"/>) AS Op2) as AuxTable)
			</xsl:when>
			<xsl:when test="@Op2Nullable = 'false'">
				(SELECT ISNULL(CAST(<xsl:apply-templates select="./*[1]"/> AS nvarchar(max)) + AuxTable.Op2, AuxTable.Op2) FROM (SELECT (<xsl:apply-templates select="./*[2]"/>) AS Op2) as AuxTable)
			</xsl:when>
			<xsl:otherwise>
				(SELECT COALESCE(CAST(AuxTable.Op1 AS nvarchar(max))+ AuxTable.Op2, AuxTable.Op1, AuxTable.Op2) FROM (SELECT (<xsl:apply-templates select="./*[1]"/>) AS Op1, (<xsl:apply-templates select="./*[2]"/>) AS Op2) as AuxTable)
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="OpMid">
		...
		<xsl:variable name="Op3">
			<xsl:choose>
				...
				<xsl:otherwise> ISNULL(<xsl:apply-templates select="./*[3]"/>, 0) </xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template match="OpSubstitute">
		<xsl:choose>
			...
			<xsl:when test="@Op3Nullable = 'false'">
				ISNULL(REPLACE(<xsl:apply-templates select="./*[1]"/>, <xsl:apply-templates select="./*[2]"/>, <xsl:apply-templates select="./*[3]"/>), N'')
			</xsl:when>
			<xsl:otherwise>
				(SELECT ISNULL(REPLACE(AuxTable.Op1, AuxTable.Op2, ISNULL(AuxTable.Op3, N'')), AuxTable.Op1)
	...
	<xsl:template match="OpReplace">
		<xsl:variable name="Op1">
			<xsl:choose>
				<xsl:when test="@Op1Nullable = 'false'"> (<xsl:apply-templates select="./*[1]"/>) </xsl:when>
				<xsl:otherwise> ISNULL(<xsl:apply-templates select="./*[1]"/>, N'') </xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="Op3">
			<xsl:choose>
		...
		<xsl:variable name="Op4">
			<xsl:choose>
				...
				<xsl:otherwise> ISNULL(<xsl:apply-templates select="./*[4]"/>, N'') </xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template match="OpSearch">
		<xsl:variable name="Op2">
			<xsl:choose>
				...
				<xsl:otherwise>
					ISNULL(<xsl:apply-templates select="./*[2]"/>, N'')
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:choose>
