...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" version="1.0" xmlns:mssqlcrt="urn:sql-microsoft-com:sqlcrt" xmlns:mssqldbg="urn:sql-microsoft-com:sqldbg">
	...
	<!-- Microsoft Analysis Server 90 (Yukon) pluggable cartridge         -->
	...
	<mssqlcrt:provider type="prefix" managed="yes" native="yes">Analysis Server</mssqlcrt:provider>
	...
	<xsl:template match="NaturalJoin">
		<xsl:for-each select="./*">
	...
	<xsl:template match="Sources">
		<xsl:choose>
			<xsl:when test="./NaturalJoin">
			FROM <xsl:apply-templates select="./*[1]"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="Count|Min|Max|Sum|Aggregate">
		<xsl:variable name="function">
			<xsl:choose>
				...
				<xsl:when test="name()='Aggregate'"> AGGREGATE </xsl:when>
			</xsl:choose>
		</xsl:variable>
	...
	<!-- Name/Key														  -->
	<xsl:template match="Name|Key|DataId|CalculatedMeasure|UnaryOperator|CustomRollup|CustomRollupProperties|SkippedLevels|MemberValue">
		<xsl:variable name="function">
			<xsl:choose>
				<xsl:when test="name()='Name'"> NAME </xsl:when>
				<xsl:when test="name()='Key'"> KEY </xsl:when>
				<xsl:when test="name()='DataId'"> DATAID </xsl:when>
				<xsl:when test="name()='CalculatedMeasure'"> CalculatedMeasure </xsl:when>
				<xsl:when test="name()='UnaryOperator'"> UnaryOperator </xsl:when>
				<xsl:when test="name()='CustomRollup'"> CustomRollup </xsl:when>
				<xsl:when test="name()='CustomRollupProperties'"> CustomRollupProperties </xsl:when>
				<xsl:when test="name()='SkippedLevels'"> SkippedLevels </xsl:when>
				<xsl:when test="name()='MemberValue'"> MemberValue </xsl:when>
			</xsl:choose>
		</xsl:variable>
		<xsl:value-of select="$function"/>( <xsl:call-template name="print-children-list"/> )
	</xsl:template>
